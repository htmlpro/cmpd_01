...
class MergeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $new_pdf = new FPDI();
            foreach ($page_num as $key => $num) {
                $new_pdf->AddPage();
                $new_pdf->setSourceFile('./public/attachments/' . $file_name);
                $new_pdf->useTemplate($new_pdf->importPage($num));
            }
            ...
            OrderManipulationHistory::create([
                'order_id' => $order_id,
                'action' => 'Merge',
                'new_orders' => $orders->id,
                'created_by' => auth()->user()->id,
                'ip' => $_SERVER['REMOTE_ADDR']
            ]);
            DB::commit();
            return trans('messages.merge_pages') . $new_order_id;
        } catch (\Exception $e) {
            DB::rollback();
